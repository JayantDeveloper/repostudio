'use client'

import Image from 'next/image'
import Link from 'next/link'
import { usePathname } from 'next/navigation'
import { useSession, signIn } from 'next-auth/react'

export function Navbar() {
  const { data: session } = useSession()
  const pathname = usePathname()
  const isActive = (href: string) => href === '/' ? pathname === '/' : pathname.startsWith(href)

  return (
    <nav className="app-navbar" style={{
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'space-between',
      gap: 16,
      padding: '0 28px',
      height: 60,
      width: '100%',
      position: 'sticky',
      top: 0,
      zIndex: 50,
    }}>
      {/* Brand: name text first, then logo */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 24, minWidth: 0 }}>
        <Link href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: 20, fontWeight: 760, color: '#f8fbff', letterSpacing: '0.01em', whiteSpace: 'nowrap' }}>
            RepoStudio
          </span>
          <Image
            src="/logo.png"
            alt=""
            width={28}
            height={28}
            style={{ objectFit: 'contain', width: 28, height: 28 }}
            priority
          />
        </Link>

        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          <Link href="/#features" className={`glass-link ${isActive('/#features') ? 'active' : ''}`}>Features</Link>
          <Link href="/docs" className={`glass-link ${isActive('/docs') ? 'active' : ''}`}>Docs</Link>
        </div>
      </div>

      {/* Right: Sign In or Dashboard */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        {session ? (
          <Link href="/dashboard">
            <button className="glass-button glass-button-primary" style={{ padding: '8px 18px', fontSize: 13 }}>
              Dashboard
            </button>
          </Link>
        ) : (
          <button
            onClick={() => signIn('github', { callbackUrl: '/dashboard' })}
            className="glass-button github-signin"
          >
            <svg aria-hidden="true" width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.44 9.8 8.2 11.38.6.1.82-.26.82-.58v-2.03c-3.34.72-4.04-1.61-4.04-1.61-.54-1.38-1.33-1.75-1.33-1.75-1.09-.74.08-.73.08-.73 1.2.09 1.84 1.24 1.84 1.24 1.07 1.83 2.8 1.3 3.49 1 .1-.78.42-1.3.76-1.6-2.67-.3-5.47-1.33-5.47-5.93 0-1.31.47-2.38 1.24-3.22-.12-.3-.54-1.52.12-3.18 0 0 1.01-.32 3.3 1.23a11.5 11.5 0 0 1 3-.4c1.02.005 2.04.14 3 .4 2.28-1.55 3.29-1.23 3.29-1.23.66 1.66.25 2.88.12 3.18.77.84 1.24 1.91 1.24 3.22 0 4.61-2.81 5.63-5.48 5.92.43.37.81 1.1.81 2.22v3.29c0 .32.22.7.83.58C20.57 21.8 24 17.3 24 12c0-6.63-5.37-12-12-12z" />
            </svg>
            <span>Sign In</span>
          </button>
        )}
      </div>
    </nav>
  )
}
